...
def remove_more_than_two_repetitions(text):
   ...
   while count < len(text):
       if text[count] != text[count-1] and text[count] != text[count-2]:
        new_sentece+=text[count]
       elif text[count] != text[count-1] and text[count] == text[count-2]:
        new_sentece+=text[count]
    #    elif count == len(text)-1 and text[count] == text[count-1]:
    #     new_sentece+=text[count]
